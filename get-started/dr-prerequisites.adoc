...
[role="lead"]
セットアップ前にストレージ、vCenter、およびコンソールの前提条件を検証して、ワークロードがレプリケーションとリカバリの準備が整った状態にします。
...
NetApp Disaster RecoveryをGoogle Cloud VMware EngineおよびGoogle Cloud NetApp Volumesと併用する場合は、適切な権限を設定し、以下の考慮事項を確認してください。

* 以下の項目を許可リストに追加するには、Google SREチームにお問い合わせください。
+
...
* フェイルオーバーは最新のスナップショットを使用した場合にのみサポートされます。必要に応じて、フェイルオーバー中に新しいスナップショットを作成できます（つまり、選択スナップショット オプションを無効にする必要があります）。
...
これらの前提条件は、ONTAP および Amazon FSx for NetApp ONTAP インスタンスに適用されます。

* ソース クラスターと宛先クラスターにはピア関係が必要です。
* ディザスタ リカバリ ボリュームをホストする SVM が宛先クラスタ上に存在している必要があります。
* ソース SVM と宛先 SVM にはピア関係が必要です。
* link:task-kubernetes-requirements.html["Kubernetes クラスタの追加要件を確認"]。
...
まだ行っていない場合は、 https://docs.netapp.com/us-en/console-setup-admin/task-sign-up-saas.html["NetApp Console に登録して組織を作成する"^]。
...
TIP: Amazon Elastic VMware Service（EVS）でディザスタリカバリを使用するには、link:../reference/evs-deploy-guide-config-prereqs.html["追加の前提条件と構成に関するガイダンス"^]を参照してください。
...
* 送信元と送信先の両方のvCenterクラスタが同じConsoleエージェントを使用していることを確認します。
* 必要なコンソール エージェントの種類:
+
** *オンプレミスからオンプレミスへのディザスタリカバリ*：ディザスタリカバリサイトにオンプレミスの Console エージェントをインストールします。この方法を使用すると、プライマリサイトに障害が発生しても、サービスが DR サイトで仮想リソースを再起動することを妨げません。 https://docs.netapp.com/us-en/console-setup-admin/task-install-connector-on-prem.html["オンプレミスでコンソールエージェントをインストールしてセットアップする"^]を参照してください。
+
...
IMPORTANT: 展開タイプに応じて、link:reference-vmware-prerequisites.html["VMware クラスタの前提条件"]とlink:task-kubernetes-requirements.html["Kubernetesクラスタの前提条件"]を確認してください。
...
* xref:../reference/vcenter-privileges.html[必須のvCenter権限]
* xref:../reference/evs-deploy-guide-config-prereqs.html[NetApp Disaster Recoveryを使用した Amazon EVS の前提条件]
