...
[role="lead"]
NetApp Disaster Recoveryを使用するには、NetApp Consoleとサポートされているディザスタリカバリ環境の両方で設定するためのいくつかの手順を実行します。
...
* *AWS Site-Site VPN*：この仮想プライベートネットワーク接続は、パブリックインターネットサービスプロバイダと連携して安全なアクセス接続を提供するために使用できます。VPNは、AWSクラウドとの間で送受信されるすべてのデータを暗号化および復号化します。VPNはソフトウェアベースまたはハードウェアベースのいずれかです。企業向けアプリケーションの場合、パブリックインターネットサービスプロバイダ（ISP）は、DRレプリケーションに必要な帯域幅とレイテンシが確保されるよう、サービス品質保証を提供する必要があります。
